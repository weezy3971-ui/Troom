    ...
    <div class="actions">
        ...
        @if($cropCycle->status === 'active')
            @if($cropCycle->template)
                <a href="{{ route('crop-cycle-templates.show', $cropCycle->template) }}" class="btn btn-secondary">🗓 {{ $cropCycle->template->crop_name }} plan</a>
            @endif
...
@php $canLog = \App\Support\ModuleAccess::allows(auth()->user(), 'daily_ops'); @endphp

<div class="card" style="margin-bottom: 20px;" id="schedule">
    <div class="card-header">
        <h3 class="card-title">Schedule</h3>
        @if($cropCycle->template)
            <p class="card-subtitle">
                <a href="{{ route('crop-cycle-templates.show', $cropCycle->template) }}">{{ $cropCycle->template->label() }}</a>
                @if($cropCycle->stageToday())
                    · currently in <strong>{{ $cropCycle->stageToday()->stage_name }}</strong>
                @endif
                @if($compliance !== null)
                    · {{ number_format($compliance * 100, 0) }}% on time
                @endif
            </p>
        @endif
    </div>
    ...
    @if(! $cropCycle->template)
        <div class="empty-state">
            <p>This cycle isn't running a template, so nothing is scheduled and no reminders will fire.</p>
            @if($canWrite)<a href="{{ route('crop-cycles.edit', $cropCycle) }}" class="btn btn-secondary btn-sm">Assign a template</a>@endif
        </div>
    @elseif($schedule->isEmpty())
        <div class="empty-state"><p>This template has no schedule points yet.</p></div>
    @elseif(! $cropCycle->planting_date)
        <div class="empty-state"><p>Set a planting date to place this template's {{ $schedule->count() }} schedule point(s) on the calendar.</p></div>
    @else
        <div class="table-wrap">
            <table>
                <thead>
                    <tr><th>Due</th><th>Day</th><th>Task</th><th>Status</th><th>Logged</th></tr>
                </thead>
                <tbody>
                    @foreach($schedule as $row)
                        ...
                        <tr>
                            ...
                            <td class="mono">{{ $row['point']->day_offset }}</td>
                            <td>{{ $row['point']->description() }}</td>
                            <td><span class="badge {{ $badge }}">{{ ucfirst($row['status']) }}</span></td>
                            <td>
                                @if($row['activity'])
                                    {{ $row['activity']->performed_date->format('d M') }}
                                    @if($row['activity']->cost_kes)
                                        · KES {{ number_format($row['activity']->cost_kes, 0) }}
                                    @endif
                                    <br><small style="color:var(--text-muted);">{{ $row['activity']->performedBy?->name ?? 'unknown' }}</small>
                                @else
                                    —
                                @endif
                            </td>
...
{{-- Activity log --}}
<div class="card" style="margin-bottom: 20px;" id="log-activity">
    <div class="card-header">
        <h3 class="card-title">Activity Log</h3>
        <p class="card-subtitle">What was actually done. Logging against a scheduled task closes it and books the cost to this cycle.</p>
    </div>

    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>Date</th><th>Activity</th><th>Product</th><th>Dosage</th><th>Cost</th><th>By</th><th>Against</th>@if($canLog)<th></th>@endif</tr>
            </thead>
            <tbody>
                @forelse($cropCycle->activities as $a)
                    <tr>
                        <td>{{ $a->performed_date->format('d M Y') }}</td>
                        <td>{{ $a->activityLabel() }}</td>
                        <td>{{ $a->product_name ?: '—' }}</td>
                        <td>{{ $a->dosage ?: '—' }}</td>
                        <td class="mono">{{ $a->cost_kes ? 'KES '.number_format($a->cost_kes, 0) : '—' }}</td>
                        <td>{{ $a->performedBy?->name ?? '—' }}</td>
                        <td>
                            @if($a->isScheduled())
                                <span class="badge badge-muted">Day {{ $a->schedulePoint->day_offset }}</span>
                                @php $late = $a->daysLate(); @endphp
                                @if($late !== null && $late > 0)<span class="badge badge-warning">{{ $late }}d late</span>@endif
                            @else
                                <span class="badge badge-muted">Ad hoc</span>
                            @endif
                        </td>
                        @if($canLog)
                            <td class="num">
                                <form action="{{ route('crop-cycles.activities.destroy', [$cropCycle, $a]) }}" method="POST"
                                      data-confirm="Remove this activity? Any task it closed will reopen.">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="{{ $canLog ? 8 : 7 }}" style="text-align:center; color: var(--text-muted); padding: 20px;">No activity logged yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if($canLog)
        <div style="padding: 16px; border-top: 1px solid var(--border);">
            <form action="{{ route('crop-cycles.activities.store', $cropCycle) }}" method="POST">
                @csrf
                <div class="form-grid">
                    <div class="form-group">
                        <label class="form-label" for="crop_cycle_schedule_point_id">Scheduled task</label>
                        <select id="crop_cycle_schedule_point_id" name="crop_cycle_schedule_point_id" class="form-select">
                            <option value="">Ad hoc — not on the plan</option>
                            @foreach($schedule as $row)
                                @continue($row['activity'] !== null)
                                <option value="{{ $row['point']->id }}" data-type="{{ $row['point']->activity_type }}">
                    ...
                    <div class="form-group">
                        <label class="form-label" for="performed_date">Date *</label>
                        <input type="date" id="performed_date" name="performed_date" class="form-input" required
                               max="{{ now()->toDateString() }}" value="{{ now()->toDateString() }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="act_product_name">Product</label>
                        <input type="text" id="act_product_name" name="product_name" class="form-input" placeholder="Defaults to what the plan called for">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="act_dosage">Dosage</label>
                        <input type="text" id="act_dosage" name="dosage" class="form-input">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="cost_kes">Cost (KES)</label>
                        <input type="number" step="0.01" min="0" id="cost_kes" name="cost_kes" class="form-input">
                        <p style="font-size:12px; color:var(--text-muted); margin-top:4px;">Booked against this cycle's budget.</p>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Picking a scheduled task sets the activity type from the plan, so
            // the two can't disagree. Left editable rather than disabled — a
            // disabled select submits nothing and would fail validation.
            var point = document.getElementById('crop_cycle_schedule_point_id');
