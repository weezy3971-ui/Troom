    ...
    <form action="{{ route('crops.store') }}" method="POST">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <x-combobox name="name" :value="old('name')" :options="\App\Support\ReferenceData::cropNames()" :required="true" placeholder="Type or pick a crop" />
            </div>
            <div class="form-group">
                <label class="form-label" for="variety">Variety</label>
                <x-combobox name="variety" :value="old('variety')" :options="\App\Support\ReferenceData::varieties()" placeholder="Type or pick a variety" />
            </div>
            <div class="form-group">
                <label class="form-label" for="crop_type">Crop Type *</label>
                <x-combobox name="crop_type" :value="old('crop_type')" :options="\App\Support\ReferenceData::cropTypes()" :required="true" placeholder="e.g. Vegetable, Fruit, Herb" />
            </div>
            <div class="form-group">
        ...
        <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--border);">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" id="seeds_per_bed" name="seeds_per_bed" value="{{ old('seeds_per_bed') }}" class="form-input" min="0">
                </div>
                <div class="form-group">
                    <label class="form-label" for="expected_yield_per_bed_kg">Expected Yield per Bed (kg)</label>
                    <input type="number" id="expected_yield_per_bed_kg" name="expected_yield_per_bed_kg" value="{{ old('expected_yield_per_bed_kg') }}" class="form-input" step="0.01" min="0">
                </div>
                <div class="form-group">
                    <label class="form-label" for="reference_price_per_kg">Reference Price (KES/kg)</label>
                    <input type="number" id="reference_price_per_kg" name="reference_price_per_kg" value="{{ old('reference_price_per_kg') }}" class="form-input" step="0.01" min="0">
                </div>
                <div class="form-group">
                    <label class="form-label" for="expected_germination_rate">Expected Germination Rate</label>
                    <input type="number" id="expected_germination_rate" name="expected_germination_rate" value="{{ old('expected_germination_rate') }}" class="form-input" step="0.01" min="0" max="1" placeholder="e.g. 0.85 for 85%">
                </div>
            </div>
        </div>
