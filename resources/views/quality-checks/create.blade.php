@extends('layouts.app')
@section('title', 'New Quality Check')

@section('content')
<div class="breadcrumbs">
    <a href="{{ route('quality-checks.index') }}">Quality Assurance</a> <span>/</span> <span>New Check</span>
</div>
<div class="page-header"><h1 class="page-title">Record Quality Check</h1></div>
...
<div class="card" style="max-width: 760px;">
    <form action="{{ route('quality-checks.store') }}" method="POST">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <select id="packhouse_lot_id" name="packhouse_lot_id" class="form-select" required>
                    ...
                    @foreach($lots as $lot)
                        <option value="{{ $lot->id }}" {{ old('packhouse_lot_id') == $lot->id ? 'selected' : '' }}>{{ $lot->lot_number }} — {{ $lot->traceability_code }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label class="form-label" for="check_date">Check Date *</label>
                <input type="date" id="check_date" name="check_date" value="{{ old('check_date', now()->toDateString()) }}" class="form-input" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="inspector_id" name="inspector_id" class="form-select">
                    ...
                    @foreach($inspectors as $user)
                        <option value="{{ $user->id }}" {{ old('inspector_id') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <textarea id="parameters" name="parameters" class="form-textarea" placeholder="Brix: 12&#10;Firmness: 4.5&#10;Defects: none">{{ old('parameters') }}</textarea>
        </div>
        <div class="form-group">
        ...
        <div style="display: flex; gap: 12px; margin-top: 8px;">
            ...
            <a href="{{ route('quality-checks.index') }}" class="btn btn-ghost">Cancel</a>
        </div>
    </form>
</div>
@endsection
