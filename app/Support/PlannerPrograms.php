...
class PlannerPrograms
{
        ...
        return [
            'french-bean' => [
                ...
                'sources' => [
                    ...
                    ['label' => 'KEPHIS: export certification & residue (MRL) requirements', 'url' => 'https://kephis.go.ke/'],
                ],
            ],
    ...
    {
        return self::matchIn(self::all(), $name);
    }

    /**
     * The same crop lookup against an already-prepared set of programs, so a
     * caller working with vetted programs doesn't have to repeat the matching.
     */
    public static function matchIn(array $programs, ?string $name): ?array
    {
        if (! $name) {
            return null;
        }
